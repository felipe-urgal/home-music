...
try {
  ...
  if (result.status === 'created') {
    ...
    console.warn(
      `[home-music] Bootstrap do primeiro administrador não executado: credencial legada inválida (${result.reason}). O login legado permanece ativo nesta etapa.`
    );
  }
} catch (error) {
  const message = error instanceof Error ? error.message : 'erro desconhecido';
  console.error(
    `[home-music] Falha no bootstrap do primeiro administrador (${message}). O servidor continuará usando o login legado nesta etapa.`
  );
}
