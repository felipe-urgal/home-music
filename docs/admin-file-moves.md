...
- a criação do novo nome usa hardlink exclusivo: `EEXIST` vira conflito `409`, inclusive se o destino surgir depois do preflight;
- depois que o hardlink aponta para o mesmo inode, o nome antigo é removido; não há janela em que os bytes dependam de cópia parcial;
- a extensão do áudio não pode ser trocada por esta operação;
- `EXDEV` é bloqueado: não há fallback para copy/delete porque isso perderia as garantias da operação no mesmo filesystem;
...
Todas as colisões de áudio e sidecars são verificadas antes da primeira alteração. O hardlink exclusivo repete a proteção contra overwrite no instante de cada movimento. Se qualquer etapa posterior falhar, os arquivos já movidos são restaurados em ordem inversa.
...
4. criar o novo hardlink sem clobber e remover o nome antigo para cada arquivo;
...
Falha antes do passo 4 não altera arquivos. Falha durante o passo 4 compensa os movimentos já concluídos. Falha entre filesystem/SQLite/índice dispara compensação para o estado anterior. Se a própria compensação falhar, a API retorna erro operacional explícito em vez de declarar sucesso parcial.

Um crash entre a criação do hardlink e a remoção do nome antigo pode deixar dois nomes apontando temporariamente para o mesmo inode, mas não sobrescreve nem perde o conteúdo original. Esse comportamento é preferível ao risco de `rename()` substituir silenciosamente um destino criado em corrida.
