...
O adapter normaliza `track/title`, `artist/creator/uploader`, `album`, `sourceId` e a URL de thumbnail retornada pela engine. Esses valores são apenas sugestões externas.
...
- conecta ao IP público já validado;
...
A URL original é transitória e não é persistida no `ImportJob`. `stderr`, paths internos e mensagens arbitrárias do processo externo não são devolvidos ao navegador. Falhas são normalizadas pelo contrato genérico de providers.

## Offline/PWA

Este PR também corrige uma regressão observada no fluxo offline: quando o servidor está inalcançável e o dispositivo já possui músicas baixadas, o app entra automaticamente na biblioteca offline uma vez por indisponibilidade. Ao escolher “Tentar conectar”, ele não força a reentrada automática até que a conectividade seja restabelecida.
